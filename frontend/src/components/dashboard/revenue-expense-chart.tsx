...
export function RevenueExpenseChart({ revenue, expenses }: RevenueExpenseChartProps) {
  const data = MONTH_LABELS_SHORT.map((month, i) => ({
    month,
    Receita: revenue[i] ?? 0,
    Despesas: expenses[i] ?? 0,
  }))
  ...
  return (
    <ResponsiveContainer width="100%" height={280}>
      <BarChart data={data} barGap={2} margin={{ top: 8, right: 8, left: 8, bottom: 0 }}>
        ...
        <Bar dataKey="Receita" fill={CHART_COLORS.blue} radius={[4, 4, 0, 0]} maxBarSize={28} />
        <Bar dataKey="Despesas" fill={CHART_COLORS.orange} radius={[4, 4, 0, 0]} maxBarSize={28} />
      </BarChart>
    </ResponsiveContainer>
  )
}
